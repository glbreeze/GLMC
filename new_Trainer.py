...
from model.metrics import ECELoss
...
class Trainer(object):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if self.args.debug > 0:
                ...
                if (epoch + 1) % 1 == 0:
                    # pdb.set_trace()
                    nc_dict = analysis(self.model, self.train_loader, self.args)
                    self.log.info('Loss:{:.3f}, Acc:{:.2f}, NC1:{:.3f}, NC2h:{:.3f}, NC2W:{:.3f}, NC3:{:.3f}, W_M_dist:{:.3f}'.format(
                        nc_dict['loss'], nc_dict['acc'], nc_dict['nc1'], nc_dict['nc2_h'], nc_dict['nc2_w'], nc_dict['nc3'], nc_dict['nc3_1']
                        # np.array2string(nc_dict['w_norm'], separator=',', formatter={'float_kind': lambda x: "%.3f" % x}),
                        # np.array2string(nc_dict['h_norm'], separator=',', formatter={'float_kind': lambda x: "%.3f" % x}),
                        # np.array2string(nc_dict['wh_cos'], separator=',', formatter={'float_kind': lambda x: "%.3f" % x})
                    ))
    ...
    def validate(self,epoch=None):
        ...
        all_preds = []
        all_targets = []
        all_outputs = torch.empty(0).to(self.device)
        ...
        with torch.no_grad():
            ...
            model_calibrator = ECELoss(n_bins=15)
            calibration = model_calibrator(all_outputs, torch.from_numpy(np.array(all_targets)).to(self.device))
            ...
            self.log.info("calibration error {:.4f}".format(calibration.item()))
            ...
            wandb.log({'val/val_acc1': top1.avg,
                       ...
                       'val/val_few': few_acc,
                       'val/calibration_error': calibration.item()},
                      step=epoch+1)
